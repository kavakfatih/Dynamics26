...
| M5-G04 | active-set minimum composition | inactive criteria equal +infinity; overlapping active rules equal mathematical minimum independent of enumeration; finite positive global fallback covers normal product domain | DEFINED / PENDING |
| M5-G05 | curvature predictor + validity domain | sagitta/normal-angle predictors agree with analytic fixtures for R>0, 0<=delta<=R, 0<=theta<=pi; invalid domain cannot propagate NaN | DEFINED / PENDING |
...
| M5-G09 | physical-domain Lipschitz gradation | field satisfies frozen minorant using physical 3D Euclidean distance; disconnected Body/Region components do not cross-propagate under first-scope policy | DEFINED / PENDING |
...
| M5-G14 | settings fingerprint | all topology/termination-affecting settings, including gradation mapping and disconnected-component propagation policy, are fingerprinted | DEFINED / PENDING |
...
## Required active-domain fixtures

M5-G04/G05/G09 must include: scoped criteria inactive outside their `GeometryEntityId` domain,
finite-positive global fallback, two disconnected physical bodies with no default cross-body
gradation propagation, physical-3D-distance gradation, and invalid `R/delta/theta` inputs that
produce a typed result rather than NaN.
